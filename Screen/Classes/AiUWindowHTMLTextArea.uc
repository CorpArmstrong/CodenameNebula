class AiUWindowHTMLTextArea expands AiUWindowDynamicTextArea;
...
function float DrawTextLine(Canvas C, AiUWindowDynamicTextRow L, float Y)
{
	...
	CurrentStyle = AiUWindowHTMLTextRow(L).StartStyle;
	...
	if(AiUWindowHTMLTextRow(L).DisplayString == "")
		SetCanvasStyle(C, CurrentStyle);
	else
	{
		while(DisplayPos < Len(AiUWindowHTMLTextRow(L).DisplayString))
		{
			i = ReadStyleText(AiUWindowHTMLTextRow(L).StyleString, StylePos, CurrentStyle);
			S = Mid(AiUWindowHTMLTextRow(L).DisplayString, DisplayPos, i);
			DisplayPos += i;
			SetCanvasStyle(C, CurrentStyle);
...
function AiUWindowDynamicTextRow SplitRowAt(AiUWindowDynamicTextRow L, int SplitPos)
{
	...
	AiUWindowHTMLTextRow(N).EndStyle = AiUWindowHTMLTextRow(L).EndStyle;
	CurrentStyle = AiUWindowHTMLTextRow(L).StartStyle;
	HTMLUpdateStyle(L.Text, CurrentStyle);
	AiUWindowHTMLTextRow(L).EndStyle = CurrentStyle;
	AiUWindowHTMLTextRow(N).StartStyle = CurrentStyle;
...
function RemoveWrap(AiUWindowDynamicTextRow L)
{
	...
	{
		AiUWindowHTMLTextRow(L).EndStyle = AiUWindowHTMLTextRow(N).EndStyle;
		N = AiUWindowDynamicTextRow(N.Next);
	}
...
function AiUWindowDynamicTextRow AddText(string NewLine)
{
	...
	if(List.Last == List)
	{
		...
		CurrentStyle = AiUWindowHTMLTextRow(List.Last).EndStyle;
	...
	while(Input != "")
	{
		...
		case "P":
			if((Output $ LeftText) != "")
			{
				...
				Output = "";
				AiUWindowHTMLTextRow(L).StartStyle = StartStyle;
				AiUWindowHTMLTextRow(L).EndStyle = CurrentStyle;
			}
			StartStyle = CurrentStyle;
			L = Super.AddText("");
...
defaultproperties
{
     ...
     RowClass=class'AiUWindow.UWindowHTMLTextRow'
