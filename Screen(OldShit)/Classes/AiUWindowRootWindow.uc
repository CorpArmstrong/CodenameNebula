//=============================================================================
// AiUWindowRootWindow - the root window.
//=============================================================================
class AiUWindowRootWindow extends AiUWindowWindow;
...
var AiUWindowWindow		MouseWindow;		// The window the mouse is over
var bool				bMouseCapture;
var float				MouseX, MouseY;
var float				OldMouseX, OldMouseY;
var AiWindowConsole		Console;
var AiUWindowWindow		FocusedWindow;
var AiUWindowWindow		KeyFocusWindow;		// window with keyboard focus
...
var AiUWindowHotkeyWindowList	HotkeyWindows;
var config float		GUIScale;
var float				RealWidth, RealHeight;
var Font				Fonts[10];
var AiUWindowLookAndFeel	LooksAndFeels[20];
...
function AiUWindowLookAndFeel GetLookAndFeel(String LFClassName)
{
	local int i;
	local class<AiUWindowLookAndFeel> LFClass;

	LFClass = class<AiUWindowLookAndFeel>(DynamicLoadObject(LFClassName, class'Class'));
...
function Created()
{
	...
	HotkeyWindows = New class'AiUWindowHotkeyWindowList';
...
{
	local AiUWindowWindow NewMouseWindow;
	local float tx, ty;
...
function CaptureMouse(optional AiUWindowWindow W)
{
	bMouseCapture = True;
	if(W != None)
		MouseWindow = W;
...
function AddHotkeyWindow(AiUWindowWindow W)
{
//	Log("Adding hotkeys for "$W);
	AiUWindowHotkeyWindowList(HotkeyWindows.Insert(class'AiUWindowHotkeyWindowList')).Window = W;
}

function RemoveHotkeyWindow(AiUWindowWindow W)
{
...
function bool HotKeyDown(int Key, float X, float Y)
{
	...
	l = AiUWindowHotkeyWindowList(HotkeyWindows.Next);
	while(l != None)
	{
...
function ChangeLookAndFeel(string NewLookAndFeel)
{
	...
	// Completely restart AiUWindow system on the next paint
