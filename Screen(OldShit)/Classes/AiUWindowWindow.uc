//=============================================================================
// AiUWindowWindow - the parent class for all Window objects
//=============================================================================
class AiUWindowWindow extends AiUWindowBase;
...
var AiUWindowWindow		ParentWindow;			// Parent window
var AiUWindowWindow		FirstChildWindow;		// First child window - bottom window first
var AiUWindowWindow		LastChildWindow;		// Last child window - WinTop window first
var AiUWindowWindow		NextSiblingWindow;		// sibling window - next window above us
var AiUWindowWindow		PrevSiblingWindow;		// previous sibling window - next window below us
var AiUWindowWindow		ActiveWindow;			// The child of ours which is currently active
var AiUWindowRootWindow	Root;					// The root window
var AiUWindowWindow		OwnerWindow;			// Some arbitary owner window
var AiUWindowWindow		ModalWindow;			// Some window we've opened modally.
...
var bool				bLeaveOnscreen;			// Window is left onscreen when AiUWindow isn't active.
var bool				bUWindowActive;			// Is AiUWindow active?
...
var AiUWindowLookAndFeel	LookAndFeel;
...
{
	local AiUWindowWindow Prev, Child;
...
final function AiUWindowWindow FindWindowUnder(float X, float Y)
{
	local AiUWindowWindow Child;
...
final function AiUWindowWindow CheckKeyFocusWindow()
{
	local AiUWindowWindow Child;
...
final function HideChildWindow(AiUWindowWindow Child)
{
	local AiUWindowWindow Window;
...
final function ShowChildWindow(AiUWindowWindow Child, optional bool bAtBack)
{
	local AiUWindowWindow W;
...
final function AiUWindowWindow CreateWindow(class<AiUWindowWindow> WndClass, float X, float Y, float W, float H, optional AiUWindowWindow OwnerW, optional bool bUnique, optional name ObjectName)
{
	local AiUWindowWindow Child;
...
final function AiUWindowWindow GetParent(class<AiUWindowWindow> ParentClass, optional bool bExactClass)
{
	local AiUWindowWindow P;
...
final function AiUWindowWindow FindChildWindow(class<AiUWindowWindow> ChildClass, optional bool bExactClass)
{
	local AiUWindowWindow Child, Found;
...
function ShowModal(AiUWindowWindow W)
{
	ModalWindow = W;
	W.ShowWindow();
	W.BringToFront();
...
function SetParent(AiUWindowWindow NewParent)
{
	HideWindow();
...
function AiUWindowMessageBox MessageBox(string Title, string Message, MessageBoxButtons Buttons, MessageBoxResult ESCResult, optional MessageBoxResult EnterResult, optional int TimeOut)
{
	local AiUWindowMessageBox W;
	local AiUWindowFramedWindow F;

	W = AiUWindowMessageBox(Root.CreateWindow(class'AiUWindowMessageBox', 100, 100, 100, 100, Self));
	W.SetupMessageBox(Title, Message, Buttons, ESCResult, EnterResult, TimeOut);
	F = AiUWindowFramedWindow(GetParent(class'AiUWindowFramedWindow'));
...
function MessageBoxDone(AiUWindowMessageBox W, MessageBoxResult Result)
{
}
