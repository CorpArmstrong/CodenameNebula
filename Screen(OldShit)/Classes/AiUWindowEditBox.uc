// AiUWindowEditBox - simple edit box, for use in other controls such as
// AiUWindowComboxBoxControl, AiUWindowEditBoxControl etc.
...
var bool		bShowCaret;
var float		Offset;
var AiUWindowDialogControl	NotifyOwner;
...
var bool		bHistory;
var bool		bKeyDown;
var AiUWindowEditBoxHistory	HistoryList;
var AiUWindowEditBoxHistory	CurrentHistory;
...
function SetHistory(bool bInHistory)
{
	...
	{
		HistoryList = new(None) class'AiUWindowEditBoxHistory';
...
function KeyDown(int Key, float X, float Y)
{
	...
	case P.EInputKey.IK_Enter:
		if(bCanEdit)
		{
			if(bHistory)
			{
				if(Value != "")
				{
					CurrentHistory = AiUWindowEditBoxHistory(HistoryList.Insert(class'AiUWindowEditBoxHistory'));
	...
	case P.EInputKey.IK_Up:
		if(bCanEdit && bHistory)
		{
			...
			{
				CurrentHistory = AiUWindowEditBoxHistory(CurrentHistory.Next);
	...
	case P.EInputKey.IK_Down:
		if(bCanEdit && bHistory)
		{
			...
			{
				CurrentHistory = AiUWindowEditBoxHistory(CurrentHistory.Prev);
				SetValue(CurrentHistory.HistoryText);
				MoveEnd();
			}
		}
